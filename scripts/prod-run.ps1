...
try {
  Write-Info "Stopping previous docker-compose (prod) if any"
  docker compose -f $composeFile down --remove-orphans | Out-Null
  Write-Ok "Docker compose down completed (prod)"
} catch {
...
Write-Info "Starting docker-compose (prod)"
docker compose -f $composeFile up -d --build | Out-Null
Write-Ok "Docker services are starting (prod)"

function Get-ComposeContainerIds($composeFilePath) {
  docker compose -f $composeFilePath ps -q
}
...
$containerIds = @(Get-ComposeContainerIds $composeFile)
if (-not $containerIds -or $containerIds.Count -eq 0) { throw "No containers found from docker compose (prod)" }
Wait-For-ContainersReady $containerIds 120
Write-Ok "All containers are ready (prod)"

Write-Info "Listing containers (prod)"
docker compose -f $composeFile ps

Write-Ok "Production-like environment is up."